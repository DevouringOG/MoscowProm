...
def export_organizations_to_excel(
    organizations: List[Organization], db: Session
) -> BytesIO:
    """
    ...
    for year in years:
        headers.append(
            f"Среднесписочная численность персонала (всего по компании), чел {year}"
        )

    for year in years:
        headers.append(
            f"Среднесписочная численность персонала, работающего в Москве, чел {year}"
        )

    for year in years:
        headers.append(
            f"Фонд оплаты труда всех сотрудников организации, тыс. руб {year}"
        )
    ...
    for year in tax_years:
        headers.append(
            f"Налоги, уплаченные в бюджет Москвы (без акцизов), тыс.руб. {year}"
        )
    ...
    for idx, org in enumerate(organizations, start=1):
        ...
        for metric in (
            ...
            .filter(OrganizationMetrics.organization_id == org.id)
            .all()
        ):
        ...
        for tax in (
            ...
            .filter(OrganizationTaxes.organization_id == org.id)
            .all()
        ):
        ...
        assets = (
            ...
            .filter(OrganizationAssets.organization_id == org.id)
            .first()
        )
        products = (
            db.query(OrganizationProducts)
            .filter(OrganizationProducts.organization_id == org.id)
            .first()
        )
        meta = (
            db.query(OrganizationMeta)
            .filter(OrganizationMeta.organization_id == org.id)
            .first()
        )
        ...
        row = [
            ...
            (
                org.registration_date.strftime("%d.%m.%Y")
        ...
        for year in years:
            ...
            row.append(
                ...
                if metric and metric.total_employees
                else ""
            )
        ...
        for year in years:
            metric = metrics_dict.get(year)
            row.append(
                metric.moscow_employees
                if metric and metric.moscow_employees
                else ""
            )
        ...
        for year in years:
            metric = metrics_dict.get(year)
            row.append(
                metric.moscow_fot if metric and metric.moscow_fot else ""
            )

        for year in years:
            metric = metrics_dict.get(year)
            row.append(
                metric.avg_salary_total
                if metric and metric.avg_salary_total
                else ""
            )
        ...
        for year in years:
            metric = metrics_dict.get(year)
            row.append(
                metric.avg_salary_moscow
                if metric and metric.avg_salary_moscow
                else ""
            )
        ...
        for year in tax_years:
            ...
            row.append(
                tax.total_taxes_moscow
                if tax and tax.total_taxes_moscow
                else ""
            )
        ...
        row.extend(
            [
                (
                    inv_2021.investments
                    if inv_2021 and inv_2021.investments
                    else ""
                ),
                (
                    inv_2022.investments
                    if inv_2022 and inv_2022.investments
                    else ""
                ),
                (
                    inv_2023.investments
                    if inv_2023 and inv_2023.investments
                    else ""
                ),
            ]
        )
        ...
        for year in export_years:
            metric = metrics_dict.get(year)
            row.append(
                metric.export_volume if metric and metric.export_volume else ""
            )
        ...
        row.extend(
            [
                ...
                (
                    assets.building_area
                    if assets and assets.building_area
                    else ""
                ),
                ...
                (
                    assets.production_area
                    if assets and assets.production_area
                    else ""
                ),
            ]
        )
        ...
        row.extend(
            [
                ...
                (
                    products.capacity_usage
                    if products and products.capacity_usage
                    else ""
                ),
                "Да" if products and products.has_export else "Нет",
                (
                    products.export_volume_last_year
                    if products and products.export_volume_last_year
                    else ""
                ),
        ...
        row.extend(
            [
                ...
                (
                    f"{meta.industry_spark or ''} / {meta.industry_directory or ''}"
                    if meta
                    else ""
                ),
