...
from app.db.models import (
    Organization, OrganizationMetrics, OrganizationTaxes, 
    OrganizationAssets, OrganizationProducts, OrganizationMeta
)
...
def process_excel_file(file_path: Path, db: Session) -> Dict[str, Any]:
    ...
    """
    logger.info("Starting Excel processing v2 (index-based)", file=str(file_path))
    ...
    for row_idx, row in enumerate(sheet.iter_rows(min_row=2, values_only=True), start=2):
        try:
            logger.info(f"Processing row {row_idx}: {len(row)} columns, INN={row[1] if len(row) > 1 else 'N/A'}")
            ...
            if existing_org:
                ...
                logger.info(f"Updating organization", inn=inn, name=safe_str(row[2]))
                ...
                org = Organization(
                    ...
                    additional_address=safe_str(row[10]),  # Адрес дополнительной площадки
                    main_industry=safe_str(row[11]),  # Основная отрасль
                    main_subindustry=safe_str(row[12]),  # Подотрасль (Основная)
                    extra_industry=safe_str(row[13]),  # Дополнительная отрасль
                    extra_subindustry=safe_str(row[14]),  # Подотрасль (Дополнительная)
                    main_okved=safe_str(row[16]),  # Основной ОКВЭД (СПАРК)
                    main_okved_name=safe_str(row[17]),  # Вид деятельности по основному ОКВЭД
                    prod_okved=safe_str(row[18]),  # Производственный ОКВЭД
                    prod_okved_name=safe_str(row[19]),  # Вид деятельности по производственному ОКВЭД
                    company_info=safe_str(row[20]),  # Общие сведения
                    company_size=safe_str(row[21]),  # Размер предприятия (итог)
                    company_size_2022=safe_str(row[22]),  # Размер предприятия (итог) 2022
                    size_by_employees=safe_str(row[23]),  # Размер предприятия (по численности)
                    size_by_employees_2022=safe_str(row[24]),  # Размер предприятия (по численности) 2022
                    size_by_revenue=safe_str(row[25]),  # Размер предприятия (по выручке)
                    size_by_revenue_2022=safe_str(row[26]),  # Размер предприятия (по выручке) 2022
                    ...
                    parent_org_inn=safe_str(row[30]),  # ИНН головной организации
                    parent_relation_type=safe_str(row[31]),  # Вид отношения головной организации
                    head_contacts=safe_str(row[32]),  # Контактные данные руководства
                    head_email=safe_str(row[33]),  # Почта руководства
                    employee_contact=safe_str(row[34]),  # Контакт сотрудника организации
                    phone=safe_str(row[35]),  # Номер телефона
                    emergency_contact=safe_str(row[36]),  # Контактные данные ответственного по ЧС
                    website=safe_str(row[37]),  # Сайт
                    email=safe_str(row[38]),  # Электронная почта
                    support_data=safe_str(row[39]),  # Данные о мерах поддержки
                    special_status=safe_str(row[40]),  # Наличие особого статуса
                ...
                db.flush()  # Get org.id
                organizations_new += 1
                logger.info(f"Created new organization", inn=inn, name=org.name)
            ...
            for year_idx, year in enumerate(range(2017, 2024)):
                ...
                investments = None
                if year == 2021:
                    investments = safe_float(row[167] if len(row) > 167 else None)
                elif year == 2022:
                    investments = safe_float(row[168] if len(row) > 168 else None)
                elif year == 2023:
                    investments = safe_float(row[169] if len(row) > 169 else None)
                
                # Export data only for 2019-2023 (indices 170-174)
                export_volume = None
                if year == 2019:
                    export_volume = safe_float(row[170] if len(row) > 170 else None)
                elif year == 2020:
                    export_volume = safe_float(row[171] if len(row) > 171 else None)
                elif year == 2021:
                    export_volume = safe_float(row[172] if len(row) > 172 else None)
                elif year == 2022:
                    export_volume = safe_float(row[173] if len(row) > 173 else None)
                elif year == 2023:
                    export_volume = safe_float(row[174] if len(row) > 174 else None)
                
                # Only create metrics if we have at least one value
                if any([revenue, profit, total_employees, moscow_employees, 
                       total_fot, moscow_fot, avg_salary_total, avg_salary_moscow,
                       investments, export_volume]):
            ...
            for year_idx, year in enumerate(range(2017, 2025)):
                ...
                if any([total_taxes_moscow, profit_tax, property_tax, land_tax, 
                       ndfl, transport_tax, other_taxes, excise]):
            ...
            if len(row) > 175:
                ...
                if has_assets:
                    ...
                    asset = OrganizationAssets(
                        organization_id=org.id,
                        property_summary=safe_str(row[175] if len(row) > 175 else None),
                        cadastral_number_land=safe_str(row[176] if len(row) > 176 else None),
                        land_area=safe_float(row[177] if len(row) > 177 else None),
                        land_usage=safe_str(row[178] if len(row) > 178 else None),
                        land_ownership_type=safe_str(row[179] if len(row) > 179 else None),
                        land_owner=safe_str(row[180] if len(row) > 180 else None),
                        cadastral_number_building=safe_str(row[181] if len(row) > 181 else None),
                        building_area=safe_float(row[182] if len(row) > 182 else None),
                        building_usage=safe_str(row[183] if len(row) > 183 else None),
                        building_type=safe_str(row[184] if len(row) > 184 else None),
                        building_ownership_type=safe_str(row[185] if len(row) > 185 else None),
                        building_owner=safe_str(row[186] if len(row) > 186 else None),
                        production_area=safe_float(row[187] if len(row) > 187 else None),
                    )
                    db.add(asset)
            ...
            if len(row) > 188:
                has_products = any([
                    row[188] if len(row) > 188 else None,  # Производимая продукция
                    row[190] if len(row) > 190 else None,  # Название (виды производимой продукции)
                ])
                ...
                if has_products:
                    ...
                    product = OrganizationProducts(
                        organization_id=org.id,
                        product_name=safe_str(row[188] if len(row) > 188 else None),  # Производимая продукция
                        standardized_product=safe_str(row[189] if len(row) > 189 else None),  # Стандартизированная продукция
                        product_types=safe_str(row[190] if len(row) > 190 else None),  # Название (виды)
                        okpd2_codes=safe_str(row[191] if len(row) > 191 else None),  # Перечень по ОКПД 2
                        product_catalog=safe_str(row[193] if len(row) > 193 else None),  # Каталог продукции
                        has_government_orders=safe_bool(row[194] if len(row) > 194 else None),  # Наличие госзаказа
                        capacity_usage=safe_str(row[195] if len(row) > 195 else None),  # Уровень загрузки
                        has_export=safe_bool(row[196] if len(row) > 196 else None),  # Наличие экспорта
                        export_volume_last_year=safe_float(row[197] if len(row) > 197 else None),  # Объем экспорта
                        export_countries=safe_str(row[198] if len(row) > 198 else None),  # Перечень государств
                        tnved_code=safe_str(row[199] if len(row) > 199 else None),  # Код ТН ВЭД
                    )
                    db.add(product)
            ...
            if len(row) > 200:
                has_meta = any([
                    row[200] if len(row) > 200 else None,  # Развитие Реестра
                    row[201] if len(row) > 201 else None,  # Отрасль промышленности
                ])
                ...
                if has_meta:
                    ...
                    meta = OrganizationMeta(
                        organization_id=org.id,
                        registry_development=safe_str(row[200] if len(row) > 200 else None),
                        industry_spark=safe_str(row[201] if len(row) > 201 else None),
                    )
                    db.add(meta)
