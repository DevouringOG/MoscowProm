...
class FNSAPIService:
    ...
    async def get_organization_by_inn(
        ...
        try:
            ...
            logger.info(
                "fns_api_success", inn=inn, org_name=normalized.get("name")
            )
            return normalized
    ...
    async def get_financial_statements(
        ...
        try:
            ...
            logger.info(
                "fns_api_bo_success",
                inn=inn,
                years=list(data[inn].keys()) if data.get(inn) else [],
            )
            return data[inn]

        except httpx.HTTPStatusError as e:
            logger.error(
                "fns_api_bo_http_error",
                inn=inn,
                status=e.response.status_code,
                error=str(e),
            )
    ...
    def _normalize_fns_data(self, fns_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if org_type == "ИП":
            ...
            # For legal entities
            normalized["name"] = data.get("НаимСокрЮЛ", "") or data.get(
                "НаимПолнЮЛ", ""
            )
            normalized["full_name"] = data.get("НаимПолнЮЛ", "") or data.get(
                "НаимСокрЮЛ", ""
            )
            ...
            if isinstance(head, dict):
                ...
                if isinstance(head_fio, dict):
                    normalized["head_name"] = (
                        f"{head_fio.get('Фамилия', '')} {head_fio.get('Имя', '')} {head_fio.get('Отчество', '')}".strip()
                    )
