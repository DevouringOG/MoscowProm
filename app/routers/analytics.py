...
async def analytics(
    ...
    try:
        ...
        all_industries = db.query(func.distinct(Organization.main_industry)).filter(
        ...
        all_years = db.query(func.distinct(OrganizationMetrics.year)).order_by(
            OrganizationMetrics.year
        ).all()
        all_years = [year[0] for year in all_years if year[0]]
        
        all_company_sizes = db.query(func.distinct(Organization.company_size)).filter(
        ...
        all_districts = db.query(func.distinct(Organization.district)).filter(
        ...
        revenue_by_industry_query = db.query(
            ...
            func.sum(OrganizationMetrics.revenue).label('revenue')
        ).join(OrganizationMetrics).filter(
            Organization.main_industry.isnot(None),
