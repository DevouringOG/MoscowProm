...
async def organization_analytics(
    ...
    return templates.TemplateResponse("organization_analytics.html", {
        "request": request,
        "organization": org,
...
async def update_organization_from_fns(organization_id: int, db: Session = Depends(get_db)):
    org = db.query(Organization).filter(Organization.id == organization_id).first()
    ...
    try:
        ...
        db.commit()
        logger.info("organization_updated_from_fns", organization_id=organization_id, updated_fields=updated_fields)
        ...
        db.rollback()
        logger.error("fns_update_failed", organization_id=organization_id, error=str(e))
...
async def import_financials_from_fns(organization_id: int, db: Session = Depends(get_db)):
    org = db.query(Organization).filter(Organization.id == organization_id).first()
    ...
    try:
        ...
        db.commit()
        logger.info("financials_imported_from_fns", organization_id=organization_id, imported_years=imported_years)
        ...
        db.rollback()
        logger.error("financials_import_failed", organization_id=organization_id, error=str(e))
