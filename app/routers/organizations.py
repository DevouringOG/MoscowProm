...
async def create_organization(org_data: OrganizationCreate, db: Session = Depends(get_db)):
    try:
        ...
        logger.info("organization_created", organization_id=new_org.id, inn=new_org.inn)
...
async def view_organization(request: Request, organization_id: int, db: Session = Depends(get_db)):
